import { g as getRenderingRef, f as forceUpdate, h, r as registerInstance, c as createEvent, H as Host, a as getElement } from './index-5406b3af.js';
import { i as isFilterBtn, F as FILTER_PROP, a as FilterButton } from './filter.button-a9f2bd58.js';
...
const OrderRenderer = ({ ref }) => {
  ...
  return (h("div", { class: "draggable-wrapper hidden", ref: e => (service.el = e) },
    ...
    h("div", { class: "drag-position", ref: e => (service.row = e) })));
};
  ...
  return [
    h("div", { class: "main-viewport" },
      h("div", { class: "viewports" },
        children,
        h("revogr-scroll-virtual", { class: "vertical", dimension: "row", viewportStore: viewports['row'].store, dimensionStore: dimensions['row'].store, ref: el => registerElement(el, 'rowScroll'), onScrollVirtual: e => onScroll(e.detail) }),
        h(OrderRenderer, { ref: orderRef }))),
    h("revogr-scroll-virtual", { class: "horizontal", dimension: "col", viewportStore: viewports['col'].store, dimensionStore: dimensions['col'].store, ref: el => registerElement(el, 'colScroll'), onScrollVirtual: e => onScroll(e.detail) }),
  ];
};
...
const RevogrRowHeaders = ({ anyView, height, rowHeaderColumn, beforeRowAdd, onScrollViewport, onElementToScroll }) => {
  ...
  return (h("revogr-viewport-scroll", Object.assign({}, { [UUID]: parent }, { contentHeight: height, contentWidth: 0, class: "rowHeaders", key: "rowHeaders", style: { minWidth: `${colSize}px` }, ref: el => onElementToScroll(el), onScrollViewport: e => onScrollViewport(e.detail) }),
    h("revogr-header", Object.assign({}, anyView.headerProp, { viewportCol: viewport.store, parent: parent, slot: "header", canResize: false })),
    dataViews));
};
...
const RevoGridComponent = class {
  ...
  render() {
    ...
    return (h(Host, Object.assign({}, { [`${UUID}`]: this.uuid }), h(RevoViewPort, { viewports: this.viewportProvider.stores, dimensions: this.dimensionProvider.stores, orderRef: e => (this.orderService = e), registerElement: (e, k) => this.scrollingService.registerElement(e, k), onScroll: details => this.scrollingService.onScroll(details) }, this.rowHeaders ? (h(RevogrRowHeaders, { height: contentHeight, anyView: columns[0], resize: this.resize, rowHeaderColumn: typeof this.rowHeaders === 'object' ? this.rowHeaders : undefined, beforeRowAdd: y => this.selectionStoreConnector.registerRow(y), onScrollViewport: e => this.scrollingService.onScroll(e, 'headerRow'), onElementToScroll: e => this.scrollingService.registerElement(e, 'headerRow') })) : (''), h(ViewPortSections, { columnFilter: !!this.filter, resize: this.resize, readonly: this.readonly, range: this.range, rowClass: this.rowClass, editors: this.editors, useClipboard: this.useClipboard, columns: columns, onEdit: detail => {
...
const RowRenderer = ({ rowClass, size, start, style, depth }, cells) => {
  return (h("div", { class: `row ${rowClass || ''}`, style: Object.assign(Object.assign({}, style), { height: `${size}px`, transform: `translateY(${start}px)`, paddingLeft: depth ? `${PADDING_DEPTH * depth}px` : undefined }) }, cells));
};
  ...
  render() {
    ...
    for (let row of rows) {
      ...
      if (isGrouping(dataRow)) {
        rowsEls.push(h(GroupingRowRenderer, Object.assign({}, row, { model: dataRow, hasExpand: this.columnService.hasGrouping })));
        continue;
      }
      /** grouping end */
      const cells = [];
      ...
      rowsEls.push(h(RowRenderer, { rowClass: rowClass, size: row.size, start: row.start }, cells));
    }
    return rowsEls;
  }
...
const RevogrHeaderComponent = class {
  ...
  render() {
    ...
    for (let col of cols) {
      ...
      cells.push(h(HeaderRenderer, { range: range, column: col, data: colData, canFilter: !!this.columnFilter, canResize: this.canResize, onResize: e => this.onResize(e, col.itemIndex), onDoubleClick: e => this.headerDblClick.emit(e), onClick: e => this.initialHeaderClick.emit(e) }));
      visibleProps[colData === null || colData === void 0 ? void 0 : colData.prop] = col.itemIndex;
    }
    return [
      h("div", { class: "group-row" }, h(ColumnGroupsRenderer, { canResize: this.canResize, visibleProps: visibleProps, groups: this.groups, dimensionCol: this.dimensionCol.state, depth: this.groupingDepth, onResize: (changedX, startIndex, endIndex) => this.onResizeGroup(changedX, startIndex, endIndex) })),
...
class AutoFillService {
  ...
  renderAutofill(range, selectionFocus) {
    ...
    return (h("div", { class: CELL_HANDLER_CLASS, style: { left: `${handlerStyle.right}px`, top: `${handlerStyle.bottom}px` }, onMouseDown: (e) => this.selectionStart(e, this.getData(), AutoFillType.autoFill) }));
  }
  get isAutoFill() {
...
const RevogrViewportScroll = class {
  ...
  render() {
    return (h(Host, { onScroll: (e) => this.onScroll('col', e) }, h("div", { class: "inner-content-table", style: { width: `${this.contentWidth}px` } }, h("div", { class: "header-wrapper", ref: e => (this.header = e) }, h("slot", { name: HEADER_SLOT })), h("div", { class: "vertical-inner", ref: el => (this.verticalScroll = el), onScroll: (e) => this.onScroll('row', e) }, h("div", { class: "content-wrapper", style: { height: `${this.contentHeight}px` } }, h("slot", { name: CONTENT_SLOT }))), h("div", { class: "footer-wrapper", ref: e => (this.footer = e) }, h("slot", { name: FOOTER_SLOT })))));
  }
  /**
...
export { RevoGridComponent as revo_grid, RevogrData as revogr_data, Edit as revogr_edit, RevogrFocus as revogr_focus, RevogrHeaderComponent as revogr_header, OrderEditor as revogr_order_editor, OverlaySelection as revogr_overlay_selection, RevogrScrollVirtual as revogr_scroll_virtual, RevogrFocus$1 as revogr_temp_range, RevogrViewportScroll as revogr_viewport_scroll };
