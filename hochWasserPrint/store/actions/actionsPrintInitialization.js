...
export default {
     ...
     * @returns {void}
     */
    parsePlotserviceCapabilities: function ({commit, dispatch}, response) {
        commit("setLayoutList", response.layouts);
        dispatch("chooseCurrentLayout", response.layouts);
    ...
    getAttributeInLayoutByName: function ({state, commit}, name) {
        if (Array.isArray(state.currentLayout?.attributes)) {
    ...
    togglePostrenderListener: function ({state, dispatch, commit}) {
        ...
        */
        if (foundVectorTileLayers.length && state.active) {
            dispatch("Alerting/addSingleAlert", i18next.t("additional:modules.tools.hochWasserPrint.vtlWarning"), {root: true});
        }

        commit("setVisibleLayer", state.visibleLayerList);

        if (state.active && state.layoutList.length !== 0 && state.visibleLayerList.length >= 1 && typeof state.eventListener === "undefined" && state.maskStarted) {
        ...
        else if (!state.active || !state.maskStarted) {
    ...
    setPrintLayers: function ({state, dispatch, commit}, scale) {
        ...
        visibleLayer.forEach(layer => {
            ...
            else if (typeof layerModel !== "undefined") {
                layerModel.setIsOutOfRange(false);
            }
        });

        hintInfo = i18next.t("additional:modules.tools.hochWasserPrint.invisibleLayer", {scale: "1: " + thousandsSeparator(scale, " ")});
    ...
    getPrintMapScales: function ({state, dispatch, commit}) {
        if (state.printService !== "plotservice") {
            ...
            if (Array.isArray(layoutMapInfo?.scales)) {
