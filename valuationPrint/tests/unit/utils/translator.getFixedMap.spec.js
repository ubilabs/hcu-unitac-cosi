...
describe("addons/valuationPrint/utils/translator.getFixedMap.js", () => {
    ...
    describe("getFixedMap", () => {
        ...
        it("should return the map configuration with configured and default value", () => {
            ...
            expect(getFixedMap(config.coordinates, config.projection, undefined, config.style, config.bbox, config.LayerIds).dpi).to.equal(200);
            expect(getFixedMap(config.coordinates, config.projection, undefined, config.style, config.bbox, config.LayerIds).bbox).to.deep.equal([545114.8, 5914269.8, 591483.1, 5957132.8]);
            expect(getFixedMap(config.coordinates, config.projection, undefined, config.style, config.bbox, config.LayerIds).projection).to.equal("EPSG:1234");
        });
        ...
        it("should return the map configuration with configured value", () => {
            ...
            expect(getFixedMap(config.coordinates, config.projection, config.dpi, config.style, config.bbox, config.LayerIds).dpi).to.equal(72);
            expect(getFixedMap(config.coordinates, config.projection, config.dpi, config.style, config.bbox, config.LayerIds).bbox).to.deep.equal([545114.8, 5914269.8, 591483.1, 5957132.8]);
            expect(getFixedMap(config.coordinates, config.projection, config.dpi, config.style, config.bbox, config.LayerIds).projection).to.equal("EPSG:1234");
        });
    });
});
