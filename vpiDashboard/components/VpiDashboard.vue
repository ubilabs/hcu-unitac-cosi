...
<template lang="html">
  <ToolTemplate :title="$t(name)" :icon="icon" :active="active" :render-to-window="renderToWindow"
    :resizable-window="resizableWindow" :initial-width="700" :deactivate-g-f-i="deactivateGFI">
    <template #toolBody>
      <div class="row h-100">
        <div class="col-12 col-md-12 col-lg-12 h-100">
          <div class="h-100">
            <!-- Tabs Component (START) -->
            <div class="tabs horizontal" role="tabs" disabled="false">
              <!-- <Tabs /> -->
              <tabs :TabItems="TabItems">
                <div slot="tab-content-0">
                  <!-- Tab Inhalt (START) -->
                  <div class="tab">
                    <div class="tab-panel h-100" role="tabpanel">
                      <div class="tab-content h100">
                        ...
                        <div class="row cards">
                          <DataCard title="Ø Individuelle Besucher pro Jahr" :number="individualVisitorsPerYear"
                            :navigation="false" />
                          <DataCard title="Ø individuelle tägliche Besucher an einem" :number="avgBestDay"
                            :navigation="true" subtitle="Samstag" />
                          <DataCard title="Ø individuelle tägliche Besucher pro Tag" :number="avgBestHour"
                            :navigation="true" subtitle="06:00 12:00" />
                          <DataCard title="Ø individuelle tägliche Besucher pro Monat in" :number="monthdata"
                            :navigation="true" subtitle="Januar" />
                          <DataCard title="Bestes Jahr" :number="2023" :navigation="false" subtitle="Jahr zu Datum" />
                        </div>
                        <div v-if="chartData === 'overview'" class="row chart bar">
                          <BarchartItem :data="chartdata.bar" />
                        </div>
                        <div v-if="chartData === 'monthlyoverview'" class="row chart bar">
                          <BarchartItem :data="chartMonthsData.bar" />
                        </div>
                        <div v-if="chartData === 'overview'" class="row chart line">
                          <LinechartItem :data="chartdata.line" />
                        </div>
                        <div v-if="chartData === 'monthlyoverview'" class="row chart line">
